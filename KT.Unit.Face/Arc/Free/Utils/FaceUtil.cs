...
namespace KT.Unit.Face.Arc.Free.Utils
{
    public class FaceUtil
    {
        ...
        public static IntPtr ExtractFeature(IntPtr pEngine, Image image, out ASF_SingleFaceInfo singleFaceInfo)
        {
            ...
            {
                image = ArcImageUtil.ScaleImage(image, image.Width, image.Height);
            }
            if (image == null)
            {
                singleFaceInfo = new ASF_SingleFaceInfo();
        ...
        /// <param name="pEngine">引擎Handle</param>
        /// <param name="image">图像</param>
        /// <returns>保存人脸特征byte[]结果</returns>
        public static (byte[] Feature, ASF_SingleFaceInfo SingleFaceInfo) ExtractFeatureBytes(IntPtr pEngine, Image image)
        {
            ...
            byte[] pFaceModel = ExtractFeatureBytes(pEngine, imageInfo, multiFaceInfo, out singleFaceInfo);
            MemoryUtil.Free(imageInfo.imgData);

            return (pFaceModel, singleFaceInfo);
        }
        ...
        /// <param name="imageInfo">图像数据</param>
        /// <param name="multiFaceInfo">人脸检测结果</param>
        /// <returns>人脸特征btye[]结果</returns>
        public static byte[] ExtractFeatureBytes(IntPtr pEngine, ImageInfo imageInfo, ASF_MultiFaceInfo multiFaceInfo, out ASF_SingleFaceInfo singleFaceInfo)
        {
            singleFaceInfo = new ASF_SingleFaceInfo();
            if (multiFaceInfo.faceRects == null)
            {
