...
ALTER TABLE `CARD_DEVICE` DROP FOREIGN KEY `FK_CARD_DEVICE_HANDLE_ELEVATOR_DEVICE_HandElevatorDeviceId`;

ALTER TABLE `HANDLE_ELEVATOR_DEVICE` DROP FOREIGN KEY `FK_HANDLE_ELEVATOR_DEVICE_PROCESSOR_ProcessorId`;

ALTER TABLE `PROCESSOR_FLOOR` DROP FOREIGN KEY `FK_PROCESSOR_FLOOR_FLOOR_FloorId`;

DROP TABLE `ELEVATOR_GROUP_RELATION_FLOOR`;

ALTER TABLE `CARD_DEVICE` DROP INDEX `IX_CARD_DEVICE_HandElevatorDeviceId`;

ALTER TABLE `FLOOR` DROP COLUMN `RealFloorId`;

ALTER TABLE `CARD_DEVICE` DROP COLUMN `HandElevatorDeviceId`;

ALTER TABLE `FLOOR` ADD `PhysicsFloor` longtext CHARACTER SET utf8mb4 NULL;

ALTER TABLE `CARD_DEVICE` MODIFY COLUMN `HandleElevatorDeviceId` varchar(255) CHARACTER SET utf8mb4 NULL;

CREATE TABLE `ELEVATOR_GROUP_FLOOR` (
    `Id` varchar(255) CHARACTER SET utf8mb4 NOT NULL,
    `Creator` longtext CHARACTER SET utf8mb4 NULL,
    `Editor` longtext CHARACTER SET utf8mb4 NULL,
    `CreatedTime` bigint NOT NULL,
    ...
    `FloorId` varchar(255) CHARACTER SET utf8mb4 NULL,
    `RealFloorId` longtext CHARACTER SET utf8mb4 NULL,
    CONSTRAINT `PK_ELEVATOR_GROUP_FLOOR` PRIMARY KEY (`Id`),
    CONSTRAINT `FK_ELEVATOR_GROUP_FLOOR_ELEVATOR_GROUP_ElevatorGroupId` FOREIGN KEY (`ElevatorGroupId`) REFERENCES `ELEVATOR_GROUP` (`Id`) ON DELETE CASCADE,
    CONSTRAINT `FK_ELEVATOR_GROUP_FLOOR_FLOOR_FloorId` FOREIGN KEY (`FloorId`) REFERENCES `FLOOR` (`Id`) ON DELETE CASCADE
);

CREATE INDEX `IX_CARD_DEVICE_HandleElevatorDeviceId` ON `CARD_DEVICE` (`HandleElevatorDeviceId`);

CREATE INDEX `IX_ELEVATOR_GROUP_FLOOR_FloorId` ON `ELEVATOR_GROUP_FLOOR` (`FloorId`);

CREATE UNIQUE INDEX `IX_ELEVATOR_GROUP_FLOOR_ElevatorGroupId_FloorId` ON `ELEVATOR_GROUP_FLOOR` (`ElevatorGroupId`, `FloorId`);

ALTER TABLE `CARD_DEVICE` ADD CONSTRAINT `FK_CARD_DEVICE_HANDLE_ELEVATOR_DEVICE_HandleElevatorDeviceId` FOREIGN KEY (`HandleElevatorDeviceId`) REFERENCES `HANDLE_ELEVATOR_DEVICE` (`Id`) ON DELETE CASCADE;

ALTER TABLE `HANDLE_ELEVATOR_DEVICE` ADD CONSTRAINT `FK_HANDLE_ELEVATOR_DEVICE_PROCESSOR_ProcessorId` FOREIGN KEY (`ProcessorId`) REFERENCES `PROCESSOR` (`Id`) ON DELETE SET NULL;

ALTER TABLE `PROCESSOR_FLOOR` ADD CONSTRAINT `FK_PROCESSOR_FLOOR_FLOOR_FloorId` FOREIGN KEY (`FloorId`) REFERENCES `FLOOR` (`Id`) ON DELETE CASCADE;

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20210520041704_v1.5.602', '3.1.7');

ALTER TABLE `HANDLE_ELEVATOR_DEVICE` ADD `CommBox` longtext CHARACTER SET utf8mb4 NULL;

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20210525083001_v1.5.603', '3.1.7');
