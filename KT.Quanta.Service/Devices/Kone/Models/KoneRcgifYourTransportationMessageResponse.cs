...
namespace KT.Quanta.Service.Devices.Kone.Requests
{
    ...
    /// </summary>
    public class KoneRcgifYourTransportationMessageResponse : KoneSerializer
    {
        public KoneRcgifYourTransportationMessageResponse()
        {
            ////Header
            ...
            ////TODOO
            //MessageId = KoneHelper.GetMessageId();
        ...
        protected override void Read(bool isLittleEndianess)
        {
            ...
            ResponseCode = ReadByte();
            PassengerId = ReadBytes(16).ToInt128(asLittleEndian: true);
            ServingElevatorId = ReadByte();
            ServingDeckId = ReadByte();
            SourceFloor = ReadByte();
            SourceSide = ReadByte();
            DepartureLobby = ReadByte();
            DestinationFloor = ReadByte();
            DestinationSide = ReadByte();
            EstimatedTimeOfElevatorArrival = ReadUnsignedShort();
            TransferFloor = ReadByte();
            TransferSide = ReadByte();
            TransferLobby = ReadByte();
            VisualGuidanceId = ReadByte();
            AcuDataSize = ReadByte();
            AcuData = ReadBytes(AcuDataSize);
        }
        ...
        protected override void Write(bool isLittleEndianess)
        {
            ...
            WriteByte(ServingElevatorId);
            WriteByte(ServingDeckId);
            WriteByte(SourceFloor);
            WriteByte(SourceSide);
            WriteByte(DepartureLobby);
            WriteByte(DestinationFloor);
            WriteByte(DestinationSide);
            WriteUnsignedShort(EstimatedTimeOfElevatorArrival);
            WriteByte(TransferFloor);
            WriteByte(TransferSide);
            WriteByte(TransferLobby);
            WriteByte(VisualGuidanceId);
            WriteByte(AcuDataSize);
            WriteBytes(AcuData);
        }
    }
}
