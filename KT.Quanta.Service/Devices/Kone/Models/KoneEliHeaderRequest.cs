...
namespace KT.Quanta.Service.Devices.Kone.Models
{
    ...
    /// </summary>
    public class KoneEliHeaderRequest : KoneSerializer
    {
        public KoneEliHeaderRequest()
        {
            MessageByteOrder = (byte)KoneEliMessageByteOrderEnum.BYTE_ORDER_LITTLE_ENDIAN.Code;
        }
        ...
        {
            if (MessageByteOrder == KoneEliMessageByteOrderEnum.BYTE_ORDER_LITTLE_ENDIAN.Code)
            {
                return true;
            }
            else if (MessageByteOrder == KoneEliMessageByteOrderEnum.BYTE_ORDER_BIG_ENDIAN.Code)
            {
                return false;
            }
            else
