...
namespace KT.Quanta.Service.Devices.Kone.Clients
{
    ...
    /// socket客户端
    /// </summary>
    public class KoneRcgifClientHost : IKoneRcgifClientHost
    {
        ...
        private readonly ILogger<KoneRcgifClientHost> _logger;
        private readonly IServiceProvider _serviceProvider;
        private readonly KoneSettings _koneSettings;

        public KoneRcgifClientHost(ILogger<KoneRcgifClientHost> logger,
             IServiceProvider serviceProvider,
            IOptions<KoneSettings> koneSettings)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(ILogger<KoneRcgifClientHost>));
        ...
        public Task InitAsync(string ipAddress, int port)
        {
            ...
            _multithreadEventLoopGroup = new MultithreadEventLoopGroup(_koneSettings.Rcgif.EventLoopGroupCount);
            ...
            _bootstrap.Group(_multithreadEventLoopGroup)
                ...
                .Handler(new ActionChannelInitializer<ISocketChannel>(channel =>
                {
                    ...
                    //编码器
                    var koneRcgifClientFrameEncoder = _serviceProvider.GetRequiredService<KoneRcgifClientFrameEncoder>();
                    pipeline.AddLast("framing-enc", koneRcgifClientFrameEncoder);

                    //解码器
                    var koneRcgifClientFrameDecoder = _serviceProvider.GetRequiredService<KoneRcgifClientFrameDecoder>();
                    pipeline.AddLast("framing-dec", koneRcgifClientFrameDecoder);

                    //处理数据
                    var koneClientHandler = _serviceProvider.GetRequiredService<IKoneRcgifClientHandler>();
        ...
        public async Task CloseAsync()
        {
            ...
            await Task.Delay(_koneSettings.Rcgif.CloseDelaySeconds * 1000);
        ...
        public async Task SendAsync(KoneRcgifHeaderRequest request)
        {
            ...
            {
                _logger.LogError(ex, "Kone false rite and flush!");
        ...
        public async Task HeartbeatAsync()
        {
            ...
            var data = new KoneRcgifHeartbeatMessageRequest();
            if (_clientChannel?.Active == true)
            {
            ...
            if (_koneSettings.Rcgif.IsNoHeartbeatReconnect
                && _receiveHeartbeatTime > 0
        ...
        public async Task NoHeartbeatReconnectAsync()
        {
            ...
            var data = new KoneRcgifDisconnectMessageRequest();
            data.Reason = (ushort)KoneRcgifDisconnectReasonEnum.NO_HEARTBEAT.Code;
            ...
            try
            {
                ...
                // 延迟连接时间 
                await Task.Delay(_koneSettings.Rcgif.ReconnectDelaySeconds * 1000);
            }
            catch (Exception ex)
            {
        ...
        private async Task DisconnectAsync()
        {
            ...
            var data = new KoneRcgifDisconnectMessageRequest();
            data.Reason = (ushort)KoneRcgifDisconnectReasonEnum.HOST_SHUTTING_DOWN.Code;
