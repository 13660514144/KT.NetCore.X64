...
{
    public class KoneEliClientFrameDecoder : ByteToMessageDecoder
    {
        private readonly ILogger<KoneEliClientFrameDecoder> _logger;
        private readonly KoneSettings _koneSettings;

        public KoneEliClientFrameDecoder(ILogger<KoneEliClientFrameDecoder> logger,
            IOptions<KoneSettings> koneSettings)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(ILogger<KoneEliClientFrameDecoder>));
        ...
        protected override void Decode(IChannelHandlerContext context, IByteBuffer input, List<object> output)
        {
            ...
            //数据过多，防攻击
            if (input.ReadableBytes > _koneSettings.Eli.DiscardMinimumLength)
            {
            ...
            //较验数据头长度
            if (input.ReadableBytes <= Helpers.KoneEliHelper.HeaderLength)
            {
            ...
            if (messageByteOrder == KoneEliMessageByteOrderEnum.BYTE_ORDER_BIG_ENDIAN.Code)
            {
                length = input.GetInt(1);
            }
            else if (messageByteOrder == KoneEliMessageByteOrderEnum.BYTE_ORDER_LITTLE_ENDIAN.Code)
            {
            ...
            //数据长度过长丢弃数据
            if (length > _koneSettings.Eli.DiscardMinimumLength)
            {
            ...
            //返回结果
            var result = new KoneEliHeaderResponse();
            result.ReadFromBuffer(byteBuffer, messageByteOrder == KoneEliMessageByteOrderEnum.BYTE_ORDER_LITTLE_ENDIAN.Code);
