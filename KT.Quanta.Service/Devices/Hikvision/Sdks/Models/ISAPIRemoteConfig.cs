...
namespace KT.Quanta.Service.Devices.Hikvision.Models
{
    public class ISAPIRemoteConfig
    {
        ...
        public ISAPIRemoteConfig(int UserID, int commandCode, string url, CHCNetSDKCard.RemoteConfigCallback callback)
        {
            this.url = url;
            this.cmd = commandCode;
        ...
        public void SetListener(CHCNetSDKCard.RemoteConfigCallback callback)
        {
            this.listener = callback;
        }
        ...
        public bool Start()
        {
            ...
            if (remoteConfigHandle == -1)
            {
                ...
                remoteConfigHandle = CHCNetSDKCard.NET_DVR_StartRemoteConfig(lUserID, (uint)cmd, ptrURL, url.Length, listener, IntPtr.Zero);
        ...
        public void Stop()
        {
            if (remoteConfigHandle != -1)
            {
                CHCNetSDKCard.NET_DVR_StopRemoteConfig(remoteConfigHandle);
        ...
        public bool Send(string input)
        {
            ...
            if (!CHCNetSDKCard.NET_DVR_SendRemoteConfig(remoteConfigHandle, (int)CHCNetSDKCard.LONG_CFG_SEND_DATA_TYPE_ENUM.ENUM_SEND_JSON_DATA, ptrData, (uint)inputlen))
            {
        ...
        private CHCNetSDKCard.RemoteConfigCallback listener;
        private string url;
        private int cmd;
