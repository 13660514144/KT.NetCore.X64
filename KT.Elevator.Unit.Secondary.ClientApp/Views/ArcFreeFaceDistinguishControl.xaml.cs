...
using KT.Elevator.Unit.Secondary.ClientApp.Service.FaceHelpers.ArcFaceFree;
using KT.Elevator.Unit.Secondary.ClientApp.Service.Helpers;
using KT.Proxy.BackendApi.Apis;
using KT.Quanta.Common.Enums;
using KT.Unit.Face.Arc.Free.Entity;
...
{
    /// <summary>
    /// ArcFreeFaceDistinguishControl.xaml 的交互逻辑
    /// </summary>
    public partial class ArcFreeFaceDistinguishControl : System.Windows.Controls.UserControl
    {
        #region 参数定义
        /// <summary>
        /// 图片最大大小
        ...
        #region 初始化
        public ArcFreeFaceDistinguishControlViewModel ViewModel { get; private set; }
        ...
        public ArcFreeFaceDistinguishControl()
        {
            InitializeComponent();
            ...
            ViewModel = ContainerHelper.Resolve<ArcFreeFaceDistinguishControlViewModel>();

            this.DataContext = ViewModel;

            this.Loaded += ArcFreeFaceDistinguishControl_Loaded;
            this.Unloaded += ArcFreeFaceDistinguishControl_Unloaded;
        ...
        private void ArcFreeFaceDistinguishControl_Loaded(object sender, RoutedEventArgs e)
        {
            //摄像头初始化
            InitVideo();
            //开启摄像头
        ...
        private void CheckFace(PaintEventArgs e)
        {
            ...
            //检测人脸，得到Rect框
            ASF_MultiFaceInfo multiFaceInfo = FaceUtil.DetectFace(_faceProvider.FaceEngine.VideoEngine, image);
            //得到最大人脸
            ...
            Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    if (_arcFaceSettings.IsCheckLiveness)
                    {
                        ...
                        //调整图片数据，非常重要
                        ImageInfo imageInfo = ArcImageUtil.ReadBMP(image);
                        if (imageInfo == null)
                        {
                            _isRGBLock = false;
                        ...
                        //RGB活体检测
                        ASF_LivenessInfo liveInfo = FaceUtil.LivenessInfo_RGB(_faceProvider.FaceEngine.VideoRGBImageEngine,
                                                                                imageInfo,
                                                                                multiFaceInfo,
                    ...
                    else if (_faceRecognitionAppSettings.FaceRecognitionType == FaceRecognitionTypeEnum.ArcFree22.Value)
                    {
                        //提取人脸特征  
                        IntPtr feature = FaceUtil.ExtractFeature(_faceProvider.FaceEngine.VideoRGBImageEngine, image, maxFace);
                        //得到比对结果
                        var result = await _faceFactory.CompareFeatureAsync(feature);
                    ...
                    {
                        _logger.LogInformation("验证未通过：找不到匹配的结果！");
        ...
        private bool LiveDetect(Bitmap irBitmap, MRECT rect, float irOffsetX, float irOffsetY, float offsetX, float offsetY)
        {
            ...
            //检测人脸，得到Rect框
            ASF_MultiFaceInfo irMultiFaceInfo = FaceUtil.DetectFace(_faceProvider.FaceEngine.VideoIRImageEngine, irBitmap);
            ...
            //IR活体检测
            ASF_LivenessInfo liveInfo = FaceUtil.LivenessInfo_IR(_faceProvider.FaceEngine.VideoIRImageEngine,
                                                                    irImageInfo,
                                                                    irMultiFaceInfo,
        ...
        /// 窗体关闭事件
        /// </summary>
        private void ArcFreeFaceDistinguishControl_Unloaded(object sender, RoutedEventArgs e)
        {
