...
namespace KT.Elevator.Unit.Secondary.ClientApp.Service.FaceHelpers.ArcFaceFree
{
    public class FaceFactory
    {
            ...
            await Task.Run(() =>
            {
                ...
                for (int i = 0; i < _arcFaceSettings.LibraryThread; i++)
                {
                    try
                    {
                        ...
                        faceProvider.FaceEngine = _arcInitHelper.InitFaceEngine();
                        _faceProviders.Add(faceProvider);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError($"初始化人脸识别失败：index:{i} ex:{ex} ");
                    }
                }
                _isInited = true;
            });
        ...
        public async Task<FaceDistinguishResult> CompareFeatureAsync(IntPtr feature)
        {
            ...
            foreach (var item in _faceProviders)
            {
                ...
                {
                    var result = await item.CompareFeatureAsync(feature);
            ...
            _logger.LogInformation($"检测人脸总结束：timeSpan:{timeSpan } ");
