...
namespace KT.Elevator.Unit.Secondary.ClientApp.Device.Hitach
{
    ...
    {     
        ...
        public string OfflineActiveFile;
        ...
        {            
            ...
            //离线激活文件
            OfflineActiveFile = AppConfigurtaionServices.Configuration["OfflineActiveFile"].ToString().Trim();
        ...
        public void _Scomm_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            ...
            lock (this)
            {
                try
                {
                    ...
                    if (_Sleep == 0)
                    {
                        _logger.LogInformation($"buffer:{string.Join("-", Buffer.ToArray())}");
                        _logger.LogInformation($"datas:{BitConverter.ToString(datas)}");
                    }
                    ...
                    if (_Sleep == 1)
                    {
                        _logger.LogInformation($"_BufByte==>{BitConverter.ToString(_BufByte)}");
                    }
                    if (_BufByteLen == 9
                            ...
                            ^ _BufByte[4] ^ _BufByte[5] ^ _BufByte[6]))
                        {
                            if ((_BufByte[6] & 0x01) == 0x00) //选层器工作正常
                            {
                                ...
                                if (N > 0 )//派梯应答应
                                {
                                    ...
                                    //_eventAggregator.GetEvent<HandledElevatorEvent>().Publish(S.Handler);
