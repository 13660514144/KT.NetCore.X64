...
namespace KT.Visitor.Common.Tools.ArcIdMatch.ArcIdSdk.Utils
{
    internal class ImageUtil
    {
        ...
        /// <param name="bottom">底部坐标</param>
        /// <returns>剪裁后的图片</returns>
        public static Bitmap CutImage(Bitmap src, int left, int top, int right, int bottom)
        {
            try
            {
                Bitmap srcBitmap = new Bitmap(src);
                Bitmap dstBitmap = srcBitmap.Clone(new Rectangle(left+40, top+20, right - left+40, bottom - top+20), PixelFormat.DontCare);
                return dstBitmap;
            }
            catch (Exception e)
            {
