...
namespace KT.Proxy.BackendApi.Apis
{
    public class BackendApiBase : HttpApiBase
    {
        ...
        public override Dictionary<string, string> BaseHeaders
        {
            get
            {
        ...
        {
            //_logger.LogInformation($"PostAsync   start=={DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")}");
            var response = await base.ExecutePostAsync(partUrl, data, appendHeaders, isHasBaseHeader);
            //_logger.LogInformation($"PostAsync   end=={DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")}");
            return GetResponseResult<T>(response);
        }
        ...
        protected async Task<VoidResponse> PostResponseAsync(string partUrl, object data, Dictionary<string, string> appendHeaders = null, bool isHasBaseHeader = true)
        {
            var response = await base.ExecutePostAsync(partUrl, data, appendHeaders, isHasBaseHeader);
        ...
        private VoidResponse GetResponse(RestResponse response)
        {
