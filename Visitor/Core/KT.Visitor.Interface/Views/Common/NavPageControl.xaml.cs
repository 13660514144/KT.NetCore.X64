...
namespace KT.Visitor.Interface.Views.Common
{
    ...
    public partial class NavPageControl : UserControl
    {
        ...
        private NavPageControlViewModel _viewModel;

        private NavPageControlViewModel ViewModel
        {
            get
            {
                if (_viewModel == null)
                {
                    _viewModel = new NavPageControlViewModel();
                }
                return _viewModel;
            }
        ...
        public void RefreshData(int? page = null)
        {
            _ = InitDataAsync(page == null ? ViewModel.Page : page.Value, ViewModel.Size);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn_PageNum_Click(object sender, RoutedEventArgs e)
        {
            int pageNumber = Convert.ToInt32(((Button)sender).Tag.ToString());
            _ = InitDataAsync(pageNumber, ViewModel.Size);
        }
        ...
        private void cb_Page_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            int? size = ConvertUtil.ToInt32(cbi?.Tag);
            if (size.HasValue && size.Value > 0 && size.Value != ViewModel.Size)
            {
                _ = InitDataAsync(1, size.Value);
            }
        }
        ...
        private void txt_JumpPage_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (page.HasValue && page.Value > 0 && page.Value != ViewModel.Page)
            {
                ...
                {
                    MessageWarnBox.Show("跳转的页面已超出范围！");
                    return;
                }
