...
namespace KT.Visitor.Interface.Views.Auth
{
    ...
    /// </summary>
    public partial class InviteAuthActivePage : Page
    {

        private InviteAuthActivePageViewModel _viewModel;
        ...
        public InviteAuthActivePage(InviteAuthActivePageViewModel viewModel,
        ...
        {
            var authTypes = _viewModel.AuthTypes.Where(x => x.IsChecked).Select(x => x.Id).ToArray();

            string faceImage = string.Empty;
            //设置人脸头像
            var takePictureVM = _viewModel.TakePictureControl.ViewModel;
