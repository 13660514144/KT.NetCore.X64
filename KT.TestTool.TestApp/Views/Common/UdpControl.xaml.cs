...
namespace KT.TestTool.TestApp.Views
{
    ...
    public partial class UdpControl : UserControl
    {
        ...
        public UdpControl(ILogger<UdpControl> logger, UdpControlViewModel viewModel, IOptions<SocketSettings> socketSettings)
        {
            InitializeComponent();
        ...
        public void SendMessage(string message)
        {
            ...
            Task.Run(() =>
            {
                _viewModel.ScrollMessage.InsertTop("UDP发送器发送数据：ip:{0} port:{0} message:{1} ", _sendPoint, message);
                try
                {
                ...
                {
                    _viewModel.ScrollMessage.InsertTop("UDP发送器发送数据失败：ip:{0} port:{1} ex:{2} ", _sendPoint.Address, _sendPoint.Port, ex);
                }
            });
        }
