...
namespace KT.Visitor.IdReader
{
    ...
    public class IDR210 : IDevice
    {
        ...
        {
            var result = IDR210SDK.Authenticate();
            if (result == 0)
            {
        ...
        {
            var result = IDR210SDK.CloseComm();
            if (result == 0)
            {
        ...
        {
            var result = IDR210SDK.InitComm(1001);
            if (result != 1)
            {
        ...
        public Person ReadContent()
        {
            ...
            var result = IDR210SDK.ReadBaseInfos(Name, Gender, Folk, BirthDay, Code, Address, Agency, ExpireStart, ExpireEnd);
            if (result != 1)
            {
            ...
            try
            {
                ...
                var imageUrl = Path.Combine(AppContext.BaseDirectory, "ReferenceFiles/IdReaderSdks/IDR210/photo.bmp");
