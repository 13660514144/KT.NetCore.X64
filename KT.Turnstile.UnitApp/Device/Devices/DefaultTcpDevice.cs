...
{
    public class DefaultTcpDevice : ISocketDevice
    {
        ...
        public DefaultTcpDevice(ILogger logger)
        {
            _logger = logger;
        }
        ...
        public Task ReceiveExecAnsyc(object data)
        {
            return Task.Run(() =>
            {
                _logger.LogInformation($"处理默认TCP设备数据：{JsonConvert.SerializeObject(data, JsonUtil.JsonPrintSettings)} ");
            });
        }
        ...
        public void Dispose()
        {
            ...
            // 关闭客户端
            _tcpClient.Close();
            _logger.LogInformation("TCP设备已成功关闭！");
        }
        ...
        public Task SendExceAnsyc(string message)
        {
            return Task.Run(() =>
            {
                _logger.LogInformation("TCP设备消息发送：ip:{0} port:{1} message:{3} ", _sendPoint.Address, _sendPoint.Port, message);
                try
                {
                ...
                {
                    _logger.LogError("TCP设备消息发送失败：ip:{0} port:{1} ex:{2} ", _sendPoint.Address, _sendPoint.Port, ex);
                }
            });
        }
