...
namespace KT.Elevator.Unit.Processor.ClientApp.Service.Helpers
{
    public class HubHelper
    {
        ...
        private async Task ConnectServerAsync()
        {
            ...
            //派梯结果返回
            MasterHub.On<HandleElevatorDisplayModel>("HandleElevatorStatus", (HandleElevatorStatus) =>
            {
                _logger.LogInformation($"HandleElevatorStatus：HandleElevatorStatus:{JsonConvert.SerializeObject(HandleElevatorStatus, JsonUtil.JsonPrintSettings)}");

                _eventAggregator.GetEvent<HandleElevatorStatusEvent>().Publish(HandleElevatorStatus);
            });
        ...
        internal async Task ManualHandleElevatorAsync(UnitManualHandleElevatorRequestModel handleElevator)
        {
            await MasterHub.InvokeAsync("ManualHandleElevator", handleElevator);
        }
